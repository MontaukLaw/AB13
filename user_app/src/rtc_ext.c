...
static void RTC_WriteByte(uint8_t const regAddr, uint8_t const *pData, uint8_t pSize);
static void RTC_ReadByte(uint8_t addr, uint8_t *pData, uint8_t pSize);
 ...
 * 初始化RTC 驱动.
 */
void ExternRTC_Init(I2C_HandleTypeDef *rtc_i2cint) {
    uint8_t temp = 0;
    timeRTC_t time;
    rtc_iic = rtc_i2cint;
    /*开始计时*/
    // temp = RTC_ReadByte(0x81);
    RTC_ReadByte(0x00, &temp, 1);
//    DBG_LOG("RTC_ReadByte:%x", temp);
    if (temp & 0x80) {
        // RTC_WP_OFF();
        // RTC_WriteByte(0x80, temp - 0x80);
        temp &= 0x7f;
        RTC_WriteByte(0x00, &temp, 1);
    }
    /*24小时模式*/
    RTC_ReadByte(0x02, &temp, 1);
    // temp = RTC_ReadByte(0x85);
    if (temp & 0x80) {
        // RTC_WP_OFF();
        // RTC_WriteByte(0x84, temp - 0x80);
        temp &= 0x7f;
        RTC_WriteByte(0x02, &temp, 1);
    }
//    RTC_WP_ON();
    osMutexDef(RTC);
...
//    HAL_I2C_Master_Transmit(rtc_iic, uint16_t TRC_DEVICE_ADDR, & 0x)
//}
...
/* Private function prototypes -----------------------------------------------*/
static void RTC_WriteByte(uint8_t const regAddr, uint8_t const *pData, uint8_t pSize) {
    HAL_StatusTypeDef  rtc_ret = HAL_ERROR;
    uint8_t rtc_wp_off = 0x00, rtc_wp_on = 0x80;
    taskENTER_CRITICAL();
    rtc_ret = HAL_I2C_Mem_Write(rtc_iic, RTC_DEVICE_ADDR, 0x07, 1, &rtc_wp_off, 1, 1000);
    if(rtc_ret != HAL_OK) {
        DBG_ERR("TRC_Write");
    }
    rtc_ret = HAL_I2C_Mem_Write(rtc_iic, RTC_DEVICE_ADDR, regAddr, 1, (uint8_t*)pData, pSize, 1000);
    if(rtc_ret != HAL_OK) {
        DBG_ERR("TRC_Write");
    }
    rtc_ret = HAL_I2C_Mem_Write(rtc_iic, RTC_DEVICE_ADDR, 0x07, 1, &rtc_wp_on, 1, 1000);
    if(rtc_ret != HAL_OK) {
        DBG_ERR("TRC_Write");
    }
    taskEXIT_CRITICAL();
...
static void RTC_ReadByte(uint8_t addr, uint8_t *pData, uint8_t pSize) {
    HAL_StatusTypeDef  rtc_ret = HAL_ERROR;
    taskENTER_CRITICAL();
    rtc_ret = HAL_I2C_Mem_Read(rtc_iic, RTC_DEVICE_ADDR, addr, 1, pData, pSize, 1000);
    if(rtc_ret != HAL_OK) {
        DBG_ERR("TRC_Read");
    }
    taskEXIT_CRITICAL();
