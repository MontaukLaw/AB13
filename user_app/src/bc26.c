...
/* Private macros ------------------------------------------------------------*/
#define GPRS_PWR_ON()                   IO_L(M2M_PWR)
#define GPRS_PWR_OFF()                  IO_H(M2M_PWR)

#define GPRS_KEY_ON()                   IO_L(M2M_RESET)
#define GPRS_KEY_OFF()                  IO_H(M2M_RESET)

// #define GPRS_LED_ON()                   IO_H(LED_GSM)
// #define GPRS_LED_OFF()                  IO_L(LED_GSM)
        ...
        switch (GPRS_Param.IP_Status) {
            ...
            case ip_status_INITIAL:
                ...
                if (GPRS_Param.ConnectAddress != NULL && GPRS_Param.ConnectPort > 0) {
                    ConnectStart(0, GPRS_Param.ConnectAddress, GPRS_Param.ConnectPort);
...
static uint16_t TCPIP_Send(uint8_t *data, uint16_t len) {
    ...
    if (data != NULL && len > 0) {
        ...
        if(hexdata != NULL) {
            ...
            GPRS_SEND_DATA((uint8_t*)tmp, strlen(tmp));
            GPRS_SEND_DATA((uint8_t*)hexdata, (len << 1));
            DBG_LOG("hexdata len: %d \r\n", (len << 1));
            GPRS_SEND_DATA("\r\n", 2);
            p = GPRS_WAIT_TOKEN("SEND OK", 10000);
            if(p != NULL) {
                sent = len;
            }
            vPortFree(hexdata);
 ...
 */
static BOOL ConnectShut(void) {
    GPRS_SEND_AT("CGACT=0,1");
    return GPRS_WAIT_ACK("NO CARRIER", 10000) ? TRUE : FALSE;
}
 ...
 */
static BOOL ConnectClose(void) {
    GPRS_SEND_AT("QICLOSE=0");
    return GPRS_WAIT_ACK("CLOSE OK", 10000) ? TRUE : FALSE;
}
...
static BOOL ConnectStart(uint8_t socketid, char *addr, uint16_t port) {
    ...
    GPRS_AT_PRINTF("QIOPEN=1,0,\"TCP\",\"%s\",\"%d\",0,0,0", addr, port);
    if (GPRS_WAIT_ACK("+QIOPEN: 0,0", 10000)) {
        r = TRUE;
        GPRS_SEND_AT("QISWTMD=0,1");
        GPRS_WAIT_ACK("OK", 100);
        GPRS_SEND_AT("QICFG=\"showlength\",1");
        GPRS_WAIT_ACK("OK", 100);
        GPRS_SEND_AT("QICFG=\"dataformat\",1,1");
        GPRS_WAIT_ACK("OK", 100);
    }
    return  r;
}
...
static IP_Status_t GetIPStatus(void) {
    uint8_t i = 0;
    char *p = NULL;
    GPRS_SEND_AT("QISTATE=0,1");
    p = GPRS_WAIT_TOKEN("+QISTATE:", 10000);
    if (p != NULL) {
        while (*p && *p++ != ',');
        while (*p && *p++ != ',');
        while (*p && *p++ != ',');
