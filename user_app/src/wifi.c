...
void WIFI_SetStation(char *ssid, char *pwd) {
    ...
    // WIFI_Param.ssid = "AFUIOT";
    // WIFI_Param.pwd = "afu123456";
}
 ...
 */
static void WIFI_ManagerPoll(void) {
    static uint32_t tsNotConnect = 0, tsElink = 0, tsStation = 0;
    /*开机管理*/
    ...
    if (WIFI_Param.status == wifi_status_socket_connected) {
        ...
        if (evt.status == osEventMessage) {
            ...
            if(len == 841){
                pTemp = p;
                for(i=0; i<len; i++){
                    WiFiTXBuffer[i] = *pTemp;
                    pTemp++;
                }
                nop();
            }
            if (len > 0) {
                DBG_LOG("Wifi sending data length: %d",len);               
                WIFI_Socket_Send(p, len);
